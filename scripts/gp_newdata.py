...
def safe_eval_expr(key: str) -> Expression:
    return eval(key, {"__builtins__": {}}, EVAL_ENV)
...
def run_one(args: argparse.Namespace, seed: int) -> None:
    ...
    def metric(_x, y, _w):
        ...
        except (OutOfDataRangeError, FloatingPointError, RuntimeError, ValueError, ZeroDivisionError, SyntaxError, NameError):
            score = -1.0
    ...
    def refresh_pool() -> None:
        candidates = sorted(cache.items(), key=lambda kv: abs(kv[1]), reverse=True)
        exprs = []
