...
RUN_ROOT=/root/autodl-tmp/gva_factor_experiments/runs_newdata/filter_xgb_suite_$(date +%Y%m%d_%H%M%S)
...
echo "START=$(date '+%F %T')"
echo "SUITE=PPO-filter-weak, GP-filter-weak, GP-filter-strong, GVA-filter-from-history, XGBoost"
...
# Fast post-processing filter from existing Full-GVA snapshots.
launch gva_filter_s012 /root/miniconda3/bin/python -u /root/alpha_1203/gva_factor_experiments/scripts/gva_filter_from_history.py \
  "${COMMON[@]}" --random_seeds=0,1,2 --method_name=gva_filter --output_root="$RUN_ROOT" \
  --source_root=/root/alpha_1203/gva_factor_experiments/runs_newdata/main_compare_20260628_211640 \
  --source_pattern='full_gva25_s{seed}' --pool_capacity=10 --filter_mode=strong --qlib_kernels=64 --device_str=auto

# PPO weak filter: positive single-IC reward, smaller candidate bank, equal-weight final pool.
...
wait_pattern 'gva_filter_s012' || STATUS=$((STATUS + $?))
wait_pattern 'ppo_filter_weak_s*' || STATUS=$((STATUS + $?))

# GP weak: positive single-IC fitness and equal-weight final pool.
for seed in 0 1 2; do
  launch gp_filter_weak_s${seed} /root/miniconda3/bin/python -u /root/alpha_1203/gva_factor_experiments/scripts/gp_filter_newdata.py \
    "${COMMON[@]}" --random_seeds=${seed} --method_name=gp_filter_weak --output_root="$RUN_ROOT" \
    --pool_capacity=10 --population_size=1000 --generations=30 --tournament_size=300 \
    --eval_every_generations=2 --pool_candidate_multiplier=5 --filter_mode=weak \
    --qlib_kernels=32 --device_str=auto --n_jobs=1
done
wait_pattern 'gp_filter_weak_s*' || STATUS=$((STATUS + $?))

# GP strong: abs(IC) fitness and MSE-optimized final pool.
for seed in 0 1 2; do
...
# XGBoost is memory-heavier; run seeds sequentially.
for seed in 0 1 2; do
