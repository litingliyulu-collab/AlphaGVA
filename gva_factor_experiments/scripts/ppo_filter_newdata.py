...
class PPOFilterPool(AlphaPoolBase):
    ...
    def try_new_expr(self, expr: Expression) -> float:
        ...
        if key in self.candidates:
            return abs(self.candidates[key][0])
        try:
        ...
        if len(self.candidates) > self.candidate_limit:
            keep = sorted(self.candidates.items(), key=lambda kv: abs(kv[1][0]), reverse=True)[: self.candidate_limit]
            self.candidates = dict(keep)
        if abs(score) > self.best_ic_ret:
            self.best_ic_ret = abs(score)
        return abs(score)

    def _selected_exprs(self) -> List[Expression]:
        selected: List[Expression] = []
        for score, expr in sorted(self.candidates.values(), key=lambda item: abs(item[0]), reverse=True):
