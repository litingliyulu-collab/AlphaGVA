...
METHOD_ORDER = ['ppo', 'critic_gva25', 'full_gva25']
METHOD_LABEL = {
    'ppo': 'PPO',
    'critic_gva25': 'Critic-GVA',
    'full_gva25': 'Full-GVA',
}
...
def summarize(report: pd.DataFrame) -> Dict[str, float]:
    ...
    evol = float(e.std(ddof=1) * math.sqrt(ann)) if n > 1 else 0.0
    return {
        'days': n,
...
def plot_curves(method_reports: Dict[str, pd.DataFrame], out_dir: Path) -> None:
    ...
    for method in METHOD_ORDER:
        ...
        plt.plot(cum.index, cum.values, label=METHOD_LABEL.get(method, method), linewidth=2)
    ...
    for method in METHOD_ORDER:
        ...
        if not reps:
            continue
        excess = pd.concat([r['excess_return'].rename(i) for i, r in enumerate(reps)], axis=1).fillna(0.0).mean(axis=1)
        cum = (1 + excess).cumprod() - 1
        plt.plot(cum.index, cum.values, label=METHOD_LABEL.get(method, method), linewidth=2)
    plt.axhline(0, color='gray', linewidth=0.8)
    plt.title('Cumulative Excess Return on Test Set')
    plt.ylabel('Cumulative Excess Return')
    plt.xlabel('Date')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_dir / 'cumulative_excess_return_by_method.png', dpi=180)
    plt.close()
    ...
    for method in METHOD_ORDER:
        ...
        dd = nav / nav.cummax() - 1
        plt.plot(dd.index, dd.values, label=METHOD_LABEL.get(method, method), linewidth=2)
    plt.title('Drawdown on Test Set')
    plt.ylabel('Drawdown')
    plt.xlabel('Date')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(out_dir / 'drawdown_by_method.png', dpi=180)
    plt.close()


def main() -> None:
    ap = argparse.ArgumentParser()
    ap.add_argument('--run_root', default='/root/alpha_1203/gva_factor_experiments/runs_newdata/main_compare_20260628_211640')
    ap.add_argument('--output_dir', default='/root/alpha_1203/gva_factor_experiments/backtests/main_compare_20260628_211640_top50')
    ...
    for method in METHOD_ORDER:
        for seed in [0, 1, 2]:
            pool_path = find_final_pool(Path(args.run_root), method, seed)
            ...
            summary.update({'method': method, 'method_label': METHOD_LABEL[method], 'seed': seed, 'pool_path': str(pool_path)})
            rows.append(summary)

    metrics = pd.DataFrame(rows)
    cols = ['method', 'method_label', 'seed', 'days', 'cum_return', 'bench_cum_return', 'excess_cum_return',
            'annual_return', 'bench_annual_return', 'excess_annual_return', 'annual_vol', 'sharpe',
            'information_ratio', 'max_drawdown', 'excess_max_drawdown', 'avg_turnover', 'avg_cost', 'win_rate', 'pool_path']
    ...
    plot_curves(method_reports, out_dir)
    ...
    print('SUMMARY')
    print(summary[[('cum_return','mean'),('annual_return','mean'),('excess_cum_return','mean'),('information_ratio','mean'),('max_drawdown','mean')]].to_string())
