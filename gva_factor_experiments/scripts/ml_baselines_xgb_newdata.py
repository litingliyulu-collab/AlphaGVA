...
def train_xgboost(args, seed: int, x_train, y_train, x_valid, y_valid, x_test):
    ...
    model.fit(x_train, y_train, eval_set=[(x_valid, y_valid)], verbose=max(args.log_every, 1))
...
def run_one(args, model_name: str, seed: int) -> None:
    ...
    if model_name == 'lightgbm':
        ...
        model, pred_test = train_xgboost(args, seed, x_train_flat, y_train_flat, x_valid_flat, y_valid_flat, x_test_flat)
    elif model_name == 'mlp':
...
def parse_args():
    ...
    parser.add_argument('--xgb_rounds', type=int, default=600)
