...
echo "[QRDQN] fresh seed0,1,2" | tee -a "$LOGDIR/qrdqn_fresh_all.log"
$PY -u "$ADV" --algos qrdqn --random_seeds 0,1,2 $COMMON \
  >> "$LOGDIR/qrdqn_fresh_all.log" 2>&1

echo "[QRDQN] done" | tee -a "$LOGDIR/qrdqn_fresh_all.log"
