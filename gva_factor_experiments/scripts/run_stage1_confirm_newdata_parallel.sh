...
RUN_ROOT=/root/alpha_1203/gva_factor_experiments/runs_newdata/stage1_confirm_$(date +%Y%m%d_%H%M%S)
...
echo "START=$(date '+%F %T')"
echo "QLIB_KERNELS=126"
echo "STEPS=8000 N_STEPS=64 PPO_EPOCHS=2"
...
launch critic_gva25_s1 \
  --random_seeds=1 \
  --method_name=critic_gva25_s1 \
  --use_custom_ppo=True \
  --custom_critic_loss=hybrid \
  --td_weight=0.9 \
  --baseline_weight=0.1 \
