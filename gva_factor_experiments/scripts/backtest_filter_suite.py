...
METHOD_LABELS = {
    ...
    'full_gva': 'Full-GVA',
    'ppo_filter': 'PPO_filter_strong',
    'ppo_filter_weak': 'PPO_filter_weak',
    'gp_filter_weak': 'GP_filter_weak',
    'gp_filter_strong': 'GP_filter_strong',
    'gva_filter': 'GVA_filter',
...
METHOD_ORDER = ['ppo_filter_weak', 'ppo_filter', 'gp_filter_weak', 'gp_filter_strong', 'gva_filter', 'xgboost', 'lightgbm', 'mlp', 'alphagen_ppo', 'critic_gva', 'full_gva']
...
def discover_inputs(args: argparse.Namespace) -> List[Dict[str, str]]:
    ...
    suite = Path(args.filter_suite_root)
    if suite.exists():
        for seed in [0, 1, 2]:
            specs.append({'method': 'ppo_filter_weak', 'seed': str(seed), 'kind': 'pool', 'path': latest_one(str(suite / f'ppo_filter_weak_s{seed}' / 'results' / '*' / 'final_pool.json'))})
            specs.append({'method': 'gp_filter_weak', 'seed': str(seed), 'kind': 'pool', 'path': latest_one(str(suite / f'gp_filter_weak_s{seed}' / 'results' / '*' / 'final_pool.json'))})
            specs.append({'method': 'gp_filter_strong', 'seed': str(seed), 'kind': 'pool', 'path': latest_one(str(suite / f'gp_filter_strong_s{seed}' / 'results' / '*' / 'final_pool.json'))})
            specs.append({'method': 'gva_filter', 'seed': str(seed), 'kind': 'pool', 'path': latest_one(str(suite / f'gva_filter_s{seed}' / 'results' / '*' / 'final_pool.json'))})
            specs.append({'method': 'xgboost', 'seed': str(seed), 'kind': 'npz', 'path': latest_one(str(suite / f'xgboost_s{seed}' / 'results' / '*' / 'pred_test.npz'))})
    return specs
...
def main() -> None:
    ...
    p.add_argument('--filter_suite_root', default='/root/autodl-tmp/gva_factor_experiments/runs_newdata/filter_xgb_suite_20260629_145720')
