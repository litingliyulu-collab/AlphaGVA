...
from ..reward import RewardContext, RewardEvaluator, RewardResult
from ..reward.evaluators import quality_score_torch
...
class LinearAlphaPool(AlphaPoolBase, metaclass=ABCMeta):
    ...
    def try_new_expr(self, expr: Expression) -> float:
        ...
        if str(expr) in self._failure_cache:
            return self.best_reward if self.reward_evaluator is not None else self.best_obj
        ...
        reward = self._calculate_reward(
            ...
            mutual_ics=ic_mut,
            pool_ic=new_ic_ret,
            original_objective=new_obj,
        )
        self._maybe_update_best(new_ic_ret, new_obj)
        if reward.total > self.best_reward:
            self.best_reward = reward.total
        return reward.total
    ...
    def _calculate_reward(
        ...
        if self.reward_evaluator is None:
            result = RewardResult(
                total=float(original_objective),
                components={"original_objective": float(original_objective)},
                diagnostics={"pool_ic": float(pool_ic), "single_ic": float(single_ic)},
            )
        else:
            result = self.reward_evaluator.evaluate(RewardContext(
                expr=expr,
                calculator=self.calculator,
                pool_exprs=self.exprs[:self.size],  # type: ignore
                pool_weights=self.weights,
                old_pool_exprs=old_pool,
                single_ic=single_ic,
                mutual_ics=mutual_ics,
                pool_ic=pool_ic,
                original_objective=original_objective,
            ))
        self.last_reward_result = result
        return result
...
class MseAlphaPool(LinearAlphaPool):
    def __init__(
        ...
        super().__init__(capacity, calculator, ic_lower_bound, reward_evaluator, device)
...
class RewardAlignedMseAlphaPool(MseAlphaPool):
    """Align admission, eviction, weight optimize and pool score with R4 reward."""

    def _uses_reward_alignment(self) -> bool:
        return (
            self.reward_evaluator is not None
            and self.reward_evaluator.config.mode.lower() not in ("original",)
        )
    ...
    def _stack_alpha_values(self, indices: Optional[Iterable[int]] = None) -> torch.Tensor:
        if indices is None:
            indices = range(self.size)
        values = [self._extra_info[i] for i in indices]
        if any(v is None for v in values):
            raise TypeError("RewardAlignedMseAlphaPool requires TensorAlphaCalculator")
        return torch.stack(values)  # type: ignore

    def _ensemble_alpha_tensor(
        self,
        ...
        alpha_values = self._stack_alpha_values(idx_list)
        if weights is None:
            weight_tensor = torch.tensor(self.weights[idx_list], device=self.device, dtype=alpha_values.dtype)
        else:
            weight_tensor = torch.tensor(weights, device=self.device, dtype=alpha_values.dtype)
        return (weight_tensor[:, None, None] * alpha_values).sum(dim=0)

    def _pool_quality(self, weights: Optional[np.ndarray] = None) -> float:
        if self.size == 0:
            return 0.0
        pool_value = self._ensemble_alpha_tensor(weights)
        quality = self.reward_evaluator.positive.evaluate(pool_value, self.calculator.target)
        return float(quality["quality_score"])

    def _evaluate_candidate_admission(
        self,
        expr: Expression,
        ic_mut_threshold: Optional[float] = 0.99,
    ) -> Tuple[float, Optional[List[float]]]:
        assert self.reward_evaluator is not None
        mutual_ics: List[float] = []
        for i in range(self.size):
            mutual_ic = self.calculator.calc_mutual_IC(expr, self.exprs[i])  # type: ignore
            if ic_mut_threshold is not None and mutual_ic > ic_mut_threshold:
                return self.calculator.calc_single_IC_ret(expr), None
            mutual_ics.append(mutual_ic)

        single_ic = self.calculator.calc_single_IC_ret(expr)
        candidate_value = self.calculator.evaluate_alpha(expr)
        result = self.reward_evaluator.evaluate(RewardContext(
            expr=expr,
            calculator=self.calculator,
            pool_exprs=[expr],
            pool_weights=[1.0],
            old_pool_exprs=self.exprs[:self.size],  # type: ignore
            single_ic=single_ic,
            mutual_ics=mutual_ics,
            pool_ic=single_ic,
            original_objective=single_ic,
            candidate_value=candidate_value,
        ))
        total = float(result.total) if result.total is not None else 0.0
        if not math.isfinite(total):
            return single_ic, None
        return total, mutual_ics
    ...
    def _calc_ics(
        ...
        if self._uses_reward_alignment():
            return self._evaluate_candidate_admission(expr, ic_mut_threshold)
        return super()._calc_ics(expr, ic_mut_threshold)

    def _calc_main_objective(self) -> Optional[float]:
        if not self._uses_reward_alignment() or self.size == 0:
            return None
        return self._pool_quality()

    def evaluate_ensemble(self) -> float:
        if self._uses_reward_alignment():
            return self._pool_quality()
        return super().evaluate_ensemble()

    def _optimize_reward_weights(
        self,
        ...
        assert self.reward_evaluator is not None
        config = self.reward_evaluator.config
        target = self.calculator.target
        weights = torch.tensor(init_weights, device=self.device, dtype=alpha_values.dtype, requires_grad=True)
        optim = torch.optim.Adam([weights], lr=lr)
        ...
        for step in count():
            ...
            loss = self._l1_alpha * torch.norm(weights, p=1) - obj
    ...
    def optimize(self, lr: float = 5e-4, max_steps: int = 10000, tolerance: int = 500) -> np.ndarray:
        ...
        alpha_values = self._stack_alpha_values()
        return self._optimize_reward_weights(alpha_values, self.weights, lr, max_steps, tolerance)

    def _score_subset(self, indices: List[int]) -> float:
        if not indices:
            return -float("inf")
        alpha_values = self._stack_alpha_values(indices)
        init = self._weights[indices].copy()
        if init.sum() == 0:
            init = np.full(len(indices), 1.0 / len(indices))
        weights = self._optimize_reward_weights(alpha_values, init)
        return self._pool_quality_with_tensors(alpha_values, weights)

    def _pool_quality_with_tensors(self, alpha_values: torch.Tensor, weights: np.ndarray) -> float:
        weight_tensor = torch.tensor(weights, device=self.device, dtype=alpha_values.dtype)
        pool_value = (weight_tensor[:, None, None] * alpha_values).sum(dim=0)
        quality = self.reward_evaluator.positive.evaluate(pool_value, self.calculator.target)
        return float(quality["quality_score"])
    ...
    def _select_eviction_index(self) -> int:
        ...
        for idx in range(self.size):
            remain = [i for i in range(self.size) if i != idx]
            score = self._score_subset(remain)
            if score > best_score:
                best_score = score
                best_idx = idx
        return best_idx
    ...
    def try_new_expr(self, expr: Expression) -> float:
        if not self._uses_reward_alignment():
            return super().try_new_expr(expr)

        ic_ret, ic_mut = self._calc_ics(expr, ic_mut_threshold=0.99)
        if ic_ret is None or ic_mut is None or not math.isfinite(float(ic_ret)):
            return 0.
        if str(expr) in self._failure_cache:
        ...
        if self.size > 1:
            ...
            if self.size > self.capacity:
                worst_idx = self._select_eviction_index()
                if worst_idx == self.capacity:
                    self._pop(worst_idx)
                    self._failure_cache.add(str(expr))
                    return self.best_reward
                removed_idx = [worst_idx]
                self._pop(worst_idx)
            new_weights = self.optimize()
            self.weights = new_weights
            self.update_history.append(AddRemoveAlphas(
        ...
        self._maybe_update_best(new_ic_ret, new_obj)
        reward_total = float(reward.total) if reward.total is not None else 0.0
    ...
    def _add_factor(
        ...
        for i in range(n):
            self._mutual_ics[i][n] = self._mutual_ics[n][i] = ic_mut[i]
        self._extra_info[n] = self._get_extra_info(expr)
        new_weight = max(ic_ret, 0.01) if n == 0 else max(float(self.weights.mean()), 0.01)
        self._weights[n] = new_weight
        self.size += 1
